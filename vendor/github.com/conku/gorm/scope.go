...
func (scope *Scope) whereSQL() (sql string) {
	var (
		...
		effectField, hasEffectField                    = scope.FieldByName("Effect") //rowei
		primaryConditions, andConditions, orConditions []string
	)

	if !scope.Search.Unscoped && hasDeletedAtField {
		//sql := fmt.Sprintf("%v.%v IS NULL", quotedTableName, scope.Quote(deletedAtField.DBName)) 	//rowei
		sql := fmt.Sprintf("%v.%v = 0", quotedTableName, scope.Quote(deletedAtField.DBName)) //
		primaryConditions = append(primaryConditions, sql)
	}

	//rowei
	if !scope.Search.Unscoped && hasEffectField {
		sql := fmt.Sprintf("%v.%v = 1", quotedTableName, scope.Quote(effectField.DBName)) //
		primaryConditions = append(primaryConditions, sql)
	}
...
// trace print sql log
func (scope *Scope) trace(t int64) {
	if len(scope.SQL) > 0 {
